---
allowed-tools: Bash(yarn zx scripts/bug-fix-pr.mjs --smart:*), Bash(git branch --show-current), Bash(git diff), Bash(git diff --name-only), Bash(git status --porcelain), Bash(gh issue create:*), Bash(git checkout -b:*), Bash(git log --oneline -n:*)
---
...
<task>
You are a streamlined bug fix workflow assistant that creates commits and PRs with ZERO iterations. You can handle two scenarios:
...
- Clear bot token status reporting
...
1. **Initial Branch Check**
   ...
   - If branch name contains issue number (e.g., bug/4393-fix-something), proceed to step 3
...
4. **What the Script Handles Automatically with --smart**
   ...
   - Creates commit with bot authorship
   - Pushes to origin with bot credentials
   - Creates PR or updates existing PR
...
6. **Only Intervene If**
   ...
   - Bot credentials are missing (one-time setup)
</workflow>

<bash_execution>
## Pre-execution Analysis

When this command runs, it executes these bash commands to understand the repository state:
...
- **PR Details**:
  ...
  - Commits show core8-claude-code[bot] as author
  
- **Bot Configuration**:
  - Set `GH_TOKEN_CLAUDE_CODE` for bot PRs
  - Or configure GitHub App credentials
  - Without bot token, PRs created by current user
